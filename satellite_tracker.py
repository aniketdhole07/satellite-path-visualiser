...
# plot geo satellites
def plotGEO(sd, title_text, c_latitude=10, c_longitude=80, savePlot=True):
	# get data		
...
def plotTrack(sats_details, title_text, save_text='out.png', c_latitude=10, c_longitude=80, savePlot=True, tracking_minutes=30):
	if(sats_details):
		satnames, satids, lats, lons = map(list, zip(*sats_details))
	# subplt
	fig, ax = plt.subplots(figsize=(10, 7))
...
# plot leo sats
def plotLEO(sd, title, c_latitude=10, c_longitude=80, savePlot=True, tracking_minutes=30):			
	satnames=[]; satids=[]; lats=[]; lons=[]; 
...
# plot nav sats	
def plotNAV(sd, title, c_latitude=10, c_longitude=80, savePlot=True, tracking_minutes=600):			
	satnames=[]; satids=[]; lats=[]; lons=[]; 
...
def plotSatellites(sd, title, satlist, c_latitude=10, c_longitude=80, savePlot=True, tracking_minutes=300):			
	sat_details=[]	
	if(satlist):
